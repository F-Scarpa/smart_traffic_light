...
void app_main(void)
{
  ...
  esp_sleep_pd_config(ESP_PD_DOMAIN_RTC_PERIPH, ESP_PD_OPTION_ON);  //needed to use ext1 correctly
  ...
  esp_sleep_enable_ext1_wakeup(mask, ESP_EXT1_WAKEUP_ANY_HIGH);   //with ext1 we can use multiple pins(mask)
                                                                  // and select
                                                                  //different wakeup modes:
                                                                    //all low
                                                                    //any high
